...
elseif(strpos($result3, "fraudulent" )) {
pan>CC:  '.$lista.'</span>/span><br>';
}
el_charges_only" )) {
    echo '#DIE</span>  </span>CC:  '.$lista.'</span>  <br>Result: SK KEY #DIE OR INVALI
elseif_key_expired" )) {
    echo '#DIE</span>  </span>CC:  '.$lista.'</span>
elseif(strpos($result1, "parameter_invalid_empty" )) {
   span>CC:  '.$lista.'</span>  TO CHE
elseif(strpos($result1, "card_notn>CC:  '.$lista.'</span>  <br>Result: CARD NOT SUPPORTED</span>
else {
    echo '#DIE</span> CC:  '.$lista.'</span>  <br>RIC DECLINE | @AforArdi </span><br>';
